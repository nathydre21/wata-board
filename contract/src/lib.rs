...
const PAYMENT_COUNTER: Symbol = Symbol::new("PAY_CNT");
const PAYMENT_RECORDS: Symbol = Symbol::new("PAY_REC");
const REFUND_REQUEST_COUNTER: Symbol = Symbol::new("REF_CNT");
const REFUND_APPROVERS: Symbol = Symbol::new("REF_APR");      // List of authorized approvers
const REFUND_APPROVAL_THRESHOLD: Symbol = Symbol::new("REF_THR"); // Number of approvals needed

// Refund storage keys
const REFUND_REQUESTS: Symbol = Symbol::new("REF_REQ");
const REFUND_ID_COUNTER: Symbol = Symbol::new("REF_CNT");
const REFUND_CONFIG: Symbol = Symbol::new("REF_CFG");
const REFUND_HISTORY: Symbol = Symbol::new("REF_HIST");
const APPROVER_STATUS: Symbol = Symbol::new("APP_STAT");
...
impl NepaBillingContract {
    ...
    pub fn pay_bill(env: Env, from: Address, token_address: Address, meter_id: String, amount: i128, memo: Option<String>, nonce: String) -> u64 {
        ...
        let nonce_key = (Symbol::new("NONCE"), from.clone(), nonce.clone());
    ...
    pub fn request_refund(env: Env, requester: Address, payment_id: u64, reason: String) -> u32 {
        ...
        // Emit event
        env.events().publish(
            (Symbol::new("refund"), Symbol::new("requested")),
    ...
    pub fn approve_refund(env: Env, approver: Address, refund_id: u32) {
        ...
        if refund_request.approvers.len() >= refund_request.required_approvals as u32 {
            ...
            // Emit approved event
            env.events().publish(
                (Symbol::new("refund"), Symbol::new("approved")),
                (refund_id, refund_request.approvers.clone())
            );
        } else {
            // Emit approval event
            env.events().publish(
                (Symbol::new("refund"), Symbol::new("approval")),
                (refund_id, approver)
            );
        }
    ...
    pub fn reject_refund(env: Env, approver: Address, refund_id: u32, rejection_reason: String) {
        ...
        env.events().publish(
            (Symbol::new("refund"), Symbol::new("rejected")),
    ...
    pub fn process_refund(env: Env, admin: Address, token_address: Address, refund_id: u32) {
        ...
        env.events().publish(
            (Symbol::new("refund"), Symbol::new("processed")),
